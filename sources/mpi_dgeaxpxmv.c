...
void
printVect (double *v, int size, int rank, int nprocs, char *modif) {
    int i;
    int nbR = size / nprocs;
    int mod = size % nprocs;
...
double *
loadVect (int n, int rank, int nprocs, char *path) {
    int nbR = n / nprocs;
    int mod = n % nprocs;
    double *v;
...
void
saveVect (double *v, int n, int rank, int nprocs, char *path) {
    int nbR = n / nprocs;
    int mod = n % nprocs;
...
void
dgaxpxmv (int size, double *m, double *v, int nprocs, int rank) {
    int i,j,k;
    ...
    MPI_Allgatherv(v, nbR, MPI_DOUBLE, vFull, recvcounts, displs, MPI_DOUBLE, MPI_COMM_WORLD);
    ...
    MPI_Allgatherv(vRes, nbR, MPI_DOUBLE, vFull, recvcounts, displs, MPI_DOUBLE, MPI_COMM_WORLD);
