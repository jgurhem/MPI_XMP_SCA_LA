...
#endif
double *
initMat (int size, int rank, int nprocs) {
...
saveMat (double *mat, int size, int rank, int nprocs, char *path) {
    ...
    if(rank < mod)
        MPI_File_write_at_all (fh, rank * nbC * size * sizeof (double), mat, nbC * size, MPI_DOUBLE, &status);
    else
        MPI_File_write_at_all (fh, (rank * nbC + mod) * size * sizeof (double), mat, nbC * size, MPI_DOUBLE, &status);
    MPI_File_close (&fh);
}

double *
initVect (int n, int rank, int nprocs) {
    int nbC = n / nprocs;
    int mod = n % nprocs;
...
saveVect (double *v, int n, int rank, int nprocs, char *path) {
    ...
    if(rank < mod)
        ...
        MPI_File_write_at_all (fh, (rank * nbC + mod) * sizeof (double), v, nbC, MPI_DOUBLE, &status);
    MPI_File_close (&fh);
}
