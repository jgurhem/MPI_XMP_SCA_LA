...
void mat_read_cyclic(int size, int rank, int nprocs, void *mat, char *path) {
  ...
  MPI_Type_create_darray(nprocs /* size */, rank, 2 /* dims */, array_size,
...
void vect_read_cyclic(int size, int rank, int nprocs, void *vect, char *path) {
  ...
  MPI_Type_create_darray(nprocs /* size */, rank, 1 /* dims */, array_size,
                         array_distrib, array_dargs, array_psizes, MPI_ORDER_C,
                         MPI_DOUBLE, &darray);
  MPI_Type_commit(&darray);
...
void mat_read_block(int size, int rank, int nprocs, void *mat, char *path) {
  ...
  if (rank < mod) {
    MPI_File_read_at_all(fh, rank * nbR * size * sizeof(double), mat,
                         nbR * size, MPI_DOUBLE, &status);
  } else {
    MPI_File_read_at_all(fh, (rank * nbR + mod) * size * sizeof(double), mat,
                         nbR * size, MPI_DOUBLE, &status);
  }
  MPI_File_close(&fh);
}
...
void mat_write_block(int size, int rank, int nprocs, void *mat, char *path) {
  ...
  MPI_File_open(MPI_COMM_WORLD, path, MPI_MODE_CREATE | MPI_MODE_WRONLY,
                MPI_INFO_NULL, &fh);
  if (rank < mod)
    MPI_File_write_at_all(fh, rank * nbR * size * sizeof(double), mat,
                          nbR * size, MPI_DOUBLE, &status);
  else
    MPI_File_write_at_all(fh, (rank * nbR + mod) * size * sizeof(double), mat,
                          nbR * size, MPI_DOUBLE, &status);
  MPI_File_close(&fh);
}
