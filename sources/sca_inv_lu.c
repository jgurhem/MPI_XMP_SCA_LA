...
double *
importMat (int size, int nb, int nc, int nr, int npcol, int nprow, char *path) {
    int world_size, rank;
    MPI_Comm_size (MPI_COMM_WORLD, &world_size);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
double *
importVect (int size, int nb, int nrhs, int nc, int nr, int npcol, int nprow, char *path) {
    int world_size, rank;
    MPI_Comm_size (MPI_COMM_WORLD, &world_size);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
int main(int argc, char** argv){
    ...
    //MPI_Status status;
    // BLACS/SCALAPACK
    int nprow, npcol, lwork, liwork, info, ic = -1, context, myrow, mycol, desca[9];
    char order = 'R';
    // MATRIX
    int nb = 1, nr, nc, lld;
    int ia = 1, ja = 1;
